Joomla.submitbutton = function(task) {
	...
	if (validator.isValid(form)) {
		...
		if (validator.validate(form.name) === false) {
			msg.push(Joomla.JText._('COM_SPORTSMANAGEMENT_ADMIN_PROJECT_CSJS_ERROR_NAME'));
		}
		if (validator.validate(form['season_id']) === false
			...
			msg.push(Joomla.JText._('COM_SPORTSMANAGEMENT_ADMIN_PROJECT_CSJS_ERROR_LEAGUE_NAME'));
		}
		if ((validator.validate(form['league_id']) === false && form['league_id'].disabled != true)
			...
			msg.push(Joomla.JText._('COM_SPORTSMANAGEMENT_ADMIN_PROJECT_CSJS_ERROR_SEASON_NAME'));
		}
		if (validator.validate(form['sports_type_id']) === false
				&& form['sports_type_id'].disabled != true
				|| (form.seasonNew && form.seasonNew.disabled == false && form.seasonNew.value == "")) {
			msg.push(Joomla.JText._('COM_SPORTSMANAGEMENT_ADMIN_PROJECT_CSJS_ERROR_SPORT_TYPE'));
		}
		if (form['joomleague_admin'] && form['joomleague_admin'].value === 0) {
			msg.push(Joomla.JText._('COM_SPORTSMANAGEMENT_ADMIN_PROJECT_CSJS_ERROR_ADMIN'));
		}
		if (form['joomleague_editor'] && form['joomleague_editor'].value === 0) {
			msg.push(Joomla.JText._('COM_SPORTSMANAGEMENT_ADMIN_PROJECT_CSJS_ERROR_MATCHDAY'));
		}
		if (form['start_time'] && validator.validate(form['start_time']) === false) {
			msg.push(Joomla.JText._('COM_SPORTSMANAGEMENT_ADMIN_PROJECT_CSJS_ERROR_MATCHTIME'));
		}
		if (form['start_date'] && validator.validate(form['start_date']) === false) {
			msg.push(Joomla.JText._('COM_SPORTSMANAGEMENT_ADMIN_PROJECT_CSJS_ERROR_MATCHDATE'));
		}
