...
class sportsmanagementModelAjax extends JModel
{
        ...
        function getProjectsBySportsTypesOptions($sports_type_id, $required = false)
        {
                $this->_db->setQuery(        "        SELECT CASE WHEN CHAR_LENGTH(p.alias) THEN CONCAT_WS(':', p.id, p.alias) ELSE p.id END AS value,
                                                                        ...
                                                                        JOIN #__jsportsmanagement_sports_type AS st ON st.id = p.sports_type_id
        ...
        function getProjectDivisionsOptions($project_id, $required = false)
        {
                $this->_db->setQuery(        "        SELECT CASE WHEN CHAR_LENGTH(d.alias) THEN CONCAT_WS(':', d.id, d.alias) ELSE d.id END AS value,
                                                                        d.name AS text
                                                                        FROM #__sportsmanagement_project_team pt
                                                                        JOIN #__sportsmanagement_division d ON d.id = pt.division_id
                                                                        JOIN #__sportsmanagement_project p ON p.id = pt.project_id
        ...
        function getProjectTeamsByDivisionOptions($project_id, $division_id=0, $required=false)
        {
                $query = "        SELECT CASE WHEN CHAR_LENGTH(t.alias) THEN CONCAT_WS(':', t.id, t.alias) ELSE t.id END AS value,
                                        ...
                                        JOIN #__sportsmanagement_team t ON t.id = tt.team_id
                                        JOIN #__sportsmanagement_project p ON p.id = tt.project_id
        ...
        function getProjectsByClubOptions($club_id, $required=false)
        {
                if($club_id == 0) {
                        ...
                        $this->_db->setQuery(        "        SELECT CASE WHEN CHAR_LENGTH(p.alias) THEN CONCAT_WS(':', p.id, p.alias) ELSE p.id END AS value,
                                                                                p.name AS text
                                                                                FROM #__sportsmanagement_project_team pt
                                                                                JOIN #__sportsmanagement_team t ON t.id = pt.team_id
                                                                                JOIN #__sportsmanagement_project p ON p.id = pt.project_id
        ...
        function getProjects()
        {
                $this->_db->setQuery(        "        SELECT CASE WHEN CHAR_LENGTH(p.alias) THEN CONCAT_WS(':', p.id, p.alias) ELSE p.id END AS value,
                                                                        p.name AS text
                                                                        FROM #__sportsmanagement_project p
                                                                        ORDER BY p.name" );
        ...
        function getProjectPlayerOptions($project_id, $required = false)
        {
                $this->_db->setQuery(        "        SELECT CASE WHEN CHAR_LENGTH(p.alias) THEN CONCAT_WS(':', p.id, p.alias) ELSE p.id END AS value,
                                                                        ...
                                                                        JOIN #__sportsmanagement_team_player tp ON tp.person_id = p.id
                                                                        JOIN #__sportsmanagement_project_team pt ON pt.id = tp.projectteam_id
                                                                        WHERE pt.project_id = " . $this->_db->Quote($project_id) . "
                                                                         AND p.published = '1'
                                                                        GROUP BY p.id
                                                                        ORDER BY text" );
        ...
        function getProjectStaffOptions($project_id, $required = false)
        {
                $this->_db->setQuery(        "        SELECT CASE WHEN CHAR_LENGTH(p.alias) THEN CONCAT_WS(':', p.id, p.alias) ELSE p.id END AS value,
                                                                        CONCAT(p.lastname, ', ', p.firstname, ' (', p.birthday, ')') AS text
                                                                        FROM #__sportsmanagement_person p
                                                                        JOIN #__sportsmanagement_team_staff ts ON ts.person_id = p.id
                                                                        JOIN #__sportsmanagement_project_team pt ON pt.id = ts.projectteam_id
                                                                        WHERE pt.project_id = " . $this->_db->Quote($project_id) . "
                                                                         AND p.published = '1'
                                                                        GROUP BY p.id
                                                                        ORDER BY text" );
        ...
        function getProjectClubOptions($project_id, $required = false)
        {
                $this->_db->setQuery(        "        SELECT CASE WHEN CHAR_LENGTH(c.alias) THEN CONCAT_WS(':', c.id, c.alias) ELSE c.id END AS value,
                                                                        c.name AS text
                                                                        FROM #__v_project_team tt
                                                                        INNER JOIN #__sportsmanagement_team t ON t.id = tt.team_id
                                                                        INNER JOIN #__sportsmanagement_club AS c ON c.id = t.club_id
                                                                        INNER JOIN #__sportsmanagement_project p ON p.id = tt.project_id
                                                                        WHERE tt.project_id = " . $this->_db->Quote($project_id) . "
                                                                        GROUP BY c.id
                                                                        ORDER BY c.name" );
                return $this->addGlobalSelectElement($this->_db->loadObjectList(), $required);
        ...
        function getProjectEventsOptions($project_id, $required = false)
        {
                $query = "         SELECT        CASE WHEN CHAR_LENGTH(et.alias) THEN CONCAT_WS(':', et.id, et.alias) ELSE et.id END AS value,
                                                        et.name AS text
                                        FROM #__sportsmanagement_eventtype as et
                                        INNER JOIN #__sportsmanagement_match_event as me ON et.id = me.event_type_id
                                        INNER JOIN #__sportsmanagement_match as m ON m.id = me.match_id
                                        INNER JOIN #__sportsmanagement_round as r ON m.round_id = r.id
        ...
        function getProjectStatOptions($project_id, $required=false)
        {
                $this->_db->setQuery(        "        SELECT CASE WHEN CHAR_LENGTH(s.alias) THEN CONCAT_WS(':', s.id, s.alias) ELSE s.id END AS value,
                                                                        s.name AS text
                                                                        FROM #__sportsmanagement_project_position AS ppos
                                                                        INNER JOIN #__sportsmanagement_position_statistic AS ps ON ps.position_id = ppos.position_id
                                                                        INNER JOIN #__sportsmanagement_statistic AS s ON s.id = ps.statistic_id
                                                                        INNER JOIN #__sportsmanagement_project p ON p.id = ppos.project_id
        ...
        function getRefereesOptions($project_id, $required = false)
        {
                $query = "        SELECT        p.id AS value,
                                        ...
                                        FROM #__sportsmanagement_person AS p
                                        INNER JOIN #__sportsmanagement_project_referee AS pr ON pr.person_id = p.id
