<?php
/**
 * Legacy compatibility bridge for the native Joomla 5/6 extension-author field.
 *
 * @version    5.6.0
 * @author     diddipoeler
 * @copyright  Copyright (C) diddipoeler
 * @license    GNU General Public License version 2 or later; see LICENSE.txt
 */
\defined('_JEXEC') or die;

use Diddipoeler\Component\SportsManagement\Administrator\Field\ExtensionauthorField;

if (!class_exists(ExtensionauthorField::class)) {
    $nativeField = JPATH_ADMINISTRATOR . '/components/com_sportsmanagement/src/Field/ExtensionauthorField.php';

    if (is_file($nativeField)) {
        require_once $nativeField;
    }
}

if (!class_exists(ExtensionauthorField::class)) {
    throw new \RuntimeException('SportsManagement native Extensionauthor field could not be loaded.', 500);
}

if (!class_exists('JFormFieldExtensionAuthor', false)) {
    class_alias(ExtensionauthorField::class, 'JFormFieldExtensionAuthor');
}
