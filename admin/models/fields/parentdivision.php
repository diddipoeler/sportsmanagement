<?php
/**
 * Legacy compatibility bridge for the native Joomla 5/6 parent division field.
 *
 * @version    5.6.0
 * @author     diddipoeler
 * @copyright  Copyright (C) diddipoeler
 * @license    GNU General Public License version 2 or later; see LICENSE.txt
 */
\defined('_JEXEC') or die;

use Diddipoeler\Component\SportsManagement\Administrator\Field\ParentdivisionField;

if (!class_exists(ParentdivisionField::class)) {
    $nativeField = JPATH_ADMINISTRATOR . '/components/com_sportsmanagement/src/Field/ParentdivisionField.php';

    if (is_file($nativeField)) {
        require_once $nativeField;
    }
}

if (!class_exists(ParentdivisionField::class)) {
    throw new \RuntimeException('SportsManagement native Parentdivision field could not be loaded.', 500);
}

if (!class_exists('JFormFieldparentdivision', false)) {
    class_alias(ParentdivisionField::class, 'JFormFieldparentdivision');
}
