<?php
/**
 * Legacy compatibility bridge for the native Joomla 5/6 season team person field.
 *
 * @version    5.6.0
 * @author     diddipoeler
 * @copyright  Copyright (C) diddipoeler
 * @license    GNU General Public License version 2 or later; see LICENSE.txt
 */
\defined('_JEXEC') or die;

use Diddipoeler\Component\SportsManagement\Administrator\Field\SeasonteampersonField;

if (!class_exists(SeasonteampersonField::class)) {
    $nativeField = JPATH_ADMINISTRATOR . '/components/com_sportsmanagement/src/Field/SeasonteampersonField.php';

    if (is_file($nativeField)) {
        require_once $nativeField;
    }
}

if (!class_exists(SeasonteampersonField::class)) {
    throw new \RuntimeException('SportsManagement native Seasonteamperson field could not be loaded.', 500);
}

if (!class_exists('JFormFieldseasonteamperson', false)) {
    class_alias(SeasonteampersonField::class, 'JFormFieldseasonteamperson');
}
