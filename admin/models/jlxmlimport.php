...
class sportsmanagementModelJLXMLImport extends BaseDatabaseModel
{
	...
	public function getData($post = array())
	{
		...
		{
			$errorFound = false;
			Factory::getApplication()->enqueueMessage(__METHOD__.' '__LINE__.Text::sprintf('COM_SPORTSMANAGEMENT_ADMIN_XML_IMPORT_ERROR', ' Load of the importfile failed:'), 'error');
			foreach (libxml_get_errors() as $error)
			{
				Factory::getApplication()->enqueueMessage(__METHOD__.' '__LINE__.Text::sprintf('COM_SPORTSMANAGEMENT_ADMIN_XML_IMPORT_ERROR', $error->message), 'error');
				$errorFound = true;
			}
			if (!$errorFound)
			{
				Factory::getApplication()->enqueueMessage(__METHOD__.' '__LINE__.Text::sprintf('COM_SPORTSMANAGEMENT_ADMIN_XML_IMPORT_ERROR', ' Unknown error :-('), 'error');
			}
		}
	...
	private function _importSportsType()
	{
		...
		else
		{
			...
			$my_text .= Text::sprintf(
				'Using existing sportstype data: %1$s',
	...
	private function _importLeague()
	{
		...
		else
		{
			$my_text .= '<span style="color:' . $this->existingInDbColor . '">';
			$my_text .= Text::sprintf(
				Text::_('COM_SPORTSMANAGEMENT_XML' . strtoupper(__FUNCTION__) . '_1'),
	...
	private function _importSeason()
	{
		...
		else
		{
			$my_text .= '<span style="color:' . $this->existingInDbColor . '">';
			$my_text .= Text::sprintf(
				'Using existing season data: %1$s',
	...
	private function _importStatistics()
	{
		...
		if (!empty($this->_dbstatisticsid))
		{
			foreach ($this->_dbstatisticsid AS $key => $id)
			{
				...
				$my_text                           .= '<span style="color:' . $this->existingInDbColor . '">';
				$my_text                           .= Text::sprintf(
					'Using existing statistic data: %1$s',
	...
	private function _importParentPositions()
	{
		...
		$my_text = '';
		if (!isset($this->_datas['parentposition']) || count($this->_datas['parentposition']) == 0)
		{
			return true;
		}
		if ((!isset($this->_newparentpositionsid) || count($this->_newparentpositionsid) == 0)
			&& (!isset($this->_dbparentpositionsid) || count($this->_dbparentpositionsid) == 0)
		)
		{
			return true;
		}
		...
		if (!empty($this->_dbparentpositionsid))
		{
			foreach ($this->_dbparentpositionsid AS $key => $id)
			{
				...
				$my_text                                .= '<span style="color:' . $this->existingInDbColor . '">';
				$my_text                                .= Text::sprintf(
					'Using existing parentposition data: %1$s',
	...
	private function _importPositions()
	{
		...
		if (!empty($this->_dbpositionsid))
		{
			foreach ($this->_dbpositionsid AS $key => $id)
			{
				...
				$my_text                          .= '<span style="color:' . $this->existingInDbColor . '">';
				$my_text                          .= Text::sprintf(
					'COM_SPORTSMANAGEMENT_XML' . strtoupper(__FUNCTION__) . '_1',
					'</span><strong>' . Text::_($this->_getObjectName('position', $id)) . '</strong>'
				);
				$my_text                          .= '<br />';
			}
	...
	{
		$my_text = '';
		if (!isset($this->_datas['positioneventtype']) || count($this->_datas['positioneventtype']) == 0)
		{
			return true;
		}
		...
		foreach ($this->_datas['positioneventtype'] as $key => $positioneventtype)
		{
			...
			)
			{
				...
				$my_text .= Text::sprintf(
						'Skipping import of PositionEventtype-ID %1$s. Old-EventID: %2$s - Old-PositionID: %3$s',
						"</span><strong>$oldID</strong><span style='color:red'>",
						"</span><strong>$oldEventID</strong><span style='color:red'>",
						"</span><strong>$oldPositionID</strong>"
					) . '<br />';
				continue;
			}
			...
			if ($object = Factory::getDbo()->loadObject())
			{
				$my_text .= '<span style="color:' . $this->existingInDbColor . '">';
				$my_text .= Text::sprintf(
					'Using existing positioneventtype data - Position: %1$s - Event: %2$s',
			...
			else
			{
				...
				else
				{
					$my_text .= '<span style="color:' . $this->storeSuccessColor . '">';
					$my_text .= Text::sprintf(
						'Created new positioneventtype data. Position: %1$s - Event: %2$s',
	...
	private function _importPlayground()
	{
		...
		$option  = Factory::getApplication()->input->getCmd('option');
		$my_text = '';
		if (!isset($this->_datas['playground']) || count($this->_datas['playground']) == 0)
		{
			return true;
		}
		if ((!isset($this->_newplaygroundid) || count($this->_newplaygroundid) == 0)
			&& (!isset($this->_dbplaygroundsid) || count($this->_dbplaygroundsid) == 0)
		)
		{
			return true;
		}
		...
		if (!empty($this->_dbplaygroundsid))
		{
			foreach ($this->_dbplaygroundsid AS $key => $id)
			{
				...
				$my_text                            .= '<span style="color:' . $this->existingInDbColor . '">';
				$my_text                            .= Text::sprintf(
					'Using existing playground data: %1$s - %2$s',
	...
	private function _importClubs()
	{
		...
		if (!isset($this->_datas['club']) || count($this->_datas['club']) == 0)
		{
			return true;
		}
		if ((!isset($this->_newclubsid) || count($this->_newclubsid) == 0)
			&& (!isset($this->_dbclubsid) || count($this->_dbclubsid) == 0)
		)
		{
			return true;
		}
			...
			foreach ($this->_newclubsid AS $key => $id)
			{
				...
				else
				{
					...
					try
					{
						...
						$my_text                      .= '<span style="color:' . $this->storeSuccessColor . '">';
						$my_text                      .= Text::sprintf(
							'Created new club data: %1$s - %2$s - %3$s',
	...
	private function _convertNewPlaygroundIDs()
	{
		...
		if (isset($this->_convertPlaygroundID) && !empty($this->_convertPlaygroundID))
		{
			...
			foreach ($this->_datas['playground'] as $key => $value)
			{
				...
				if ($p_playground->club_id != $new_club_id)
				{
					if ($this->_updatePlaygroundRecord($new_club_id, $new_pg_id))
					{
						...
						$my_text   .= Text::sprintf(
							'Converted club-info %1$s in imported playground %2$s - [club_id: %3$s] [playground-id: %4$s]',
			...
			if (!$converted)
			{
				$my_text .= '<span style="color:green">' . Text::_('Nothing needed to be converted') . '<br />';
			}
		...
		if (!empty($this->_dbteamsid))
		{
			...
			foreach ($this->_dbteamsid AS $key => $id)
			{
				if (empty($this->_newteams[$key]))
				{
					...
					$my_text .= '<span style="color:' . $this->existingInDbColor . '">';
					$my_text .= Text::sprintf(
						'Using existing team data 1: %1$s - %2$s - %3$s - %4$s <- %5$s',
			...
			foreach ($this->_newteams AS $key => $value)
			{
				...
				else
				{
					...
					try
					{
						...
						$my_text  .= '<span style="color:' . $this->storeSuccessColor . '">';
						$my_text  .= Text::sprintf(
							'Created new team data: %1$s - %2$s - %3$s - %4$s - club_id [%5$s]',
	...
	private function _importPersons()
	{
		...
		if (!empty($this->_dbpersonsid))
		{
			foreach ($this->_dbpersonsid AS $key => $id)
			{
				...
				$my_text                        .= '<span style="color:' . $this->existingInDbColor . '">';
				$my_text                        .= Text::sprintf(
					'Using existing person data: %1$s',
		...
		if (!empty($this->_newpersonsid))
		{
			foreach ($this->_newpersonsid AS $key => $id)
			{
				...
				if ($object = Factory::getDbo()->loadObject())
				{
					...
					$my_text                        .= '<span style="color:' . $this->existingInDbColor . '">';
					$my_text                        .= Text::sprintf(
						'Using existing person data: %1$s %2$s [%3$s] - %4$s',
	...
	{
		$my_text = '';
		if (!isset($this->_datas['division']) || count($this->_datas['division']) == 0)
		{
			return true;
		}
	...
	private function _importProjectPositions()
	{
		...
		if (!isset($this->_datas['projectposition']) || count($this->_datas['projectposition']) == 0)
		{
			return true;
		}
		if (!isset($this->_datas['position']) || count($this->_datas['position']) == 0)
		...
		foreach ($this->_datas['projectposition'] as $key => $projectposition)
		{
			...
			if (!isset($this->_convertPositionID[$oldPositionID]))
			{
				$my_text .= '<span style="color:red">';
				$my_text .= Text::sprintf(
						'Skipping import of ProjectPosition-ID %1$s. Old-PositionID: %2$s',
						"</span><strong>$oldID</strong><span style='color:red'>",
						"</span><strong>$oldPositionID</strong>"
					) . '<br />';
				continue;
			...
			else
			{
				...
				try
				{
					...
					$my_text                                 .= '<span style="color:' . $this->storeSuccessColor . '">';
					$my_text                                 .= Text::sprintf(
						'Created new projectposition data: %1$s - %2$s',
	...
	{
		$my_text = '';
		if (!isset($this->_datas['projectreferee']) || count($this->_datas['projectreferee']) == 0)
		{
			return true;
		}
	...
	private function _importProjectTeam()
	{
		...
		foreach ($this->_datas['projectteam'] as $key => $projectteam)
		{
			...
			try
			{
				...
				$my_text           .= '<span style="color:' . $this->storeSuccessColor . '">';
				$my_text           .= Text::sprintf(
					'Created new projectteam data: %1$s - Team ID : %2$s',
		...
		foreach ($this->_datas['teamplayer'] as $key => $teamplayer)
		{
			...
			)
			{
				...
				$my_text .= Text::sprintf(
						...
						"</span><strong>$oldPersonID</strong>"
					) . '<br />';
				continue;
			}
			...
			try
			{
				...
				if ($project_position_id > 0)
				{
					...
					$my_text     .= '<span style="color:' . $this->storeSuccessColor . '">';
					$my_text     .= Text::sprintf(
						'COM_SPORTSMANAGEMENT_XML' . strtoupper(__FUNCTION__) . '_2',
	...
	private function _importTeamStaff()
	{
		...
		foreach ($this->_datas['teamstaff'] as $key => $teamstaff)
		{
			...
			)
			{
				$my_text .= '<span style="color:red">';
				$my_text .= Text::sprintf(
						'Skipping import of TeamStaff-ID %1$s. Old-ProjectTeamID: %2$s - Old-PersonID: %3$s',
						"</span><strong>$oldID</strong><span style='color:red'>",
			...
			try
			{
				...
				if ($project_position_id > 0)
				{
					...
					$my_text     .= '<span style="color:' . $this->storeSuccessColor . '">';
					$my_text     .= Text::sprintf(
						'Created new teamstaff data. Team: %1$s - Person: %2$s,%3$s - Position: %4$s',
		...
		foreach ($this->_datas['teamtraining'] as $key => $teamtraining)
		{
			...
			if (!Factory::getDbo()->loadResult())
			{
				...
				else
				{
					$my_text .= '<span style="color:' . $this->storeSuccessColor . '">';
					$my_text .= Text::sprintf(
						'Created new teamtraining data. Team: [%1$s]',
		...
		foreach ($this->_datas['match'] as $key => $match)
		{
			...
			try
			{
				...
				if ($this->import_version == 'OLD')
				{
					...
					$my_text .= '<span style="color:' . $this->storeSuccessColor . '">';
					$my_text .= Text::sprintf(
						'Added to round: %1$s / Match: %2$s - %3$s',
						'</span><strong>' . $this->_getRoundName($this->_convertRoundID[$this->_getDataFromObject($match, 'round_id')]) . '</strong><span style="color:' . $this->storeSuccessColor . '">',
						"</span><strong>$teamname1</strong>",
						"<strong>$teamname2</strong>"
					);
	...
	private function _importMatchPlayer()
	{
		...
		foreach ($this->_datas['matchplayer'] as $key => $matchplayer)
		{
			...
			)
			{
				$my_text .= '<span style="color:red">';
				$my_text .= Text::sprintf(
						'Skipping import of MatchPlayer-ID [%1$s]. Old-MatchID: [%2$s] - Old-TeamPlayerID: [%3$s]',
						"</span><strong>$oldID</strong><span style='color:red'>",
			...
			try
			{
				...
				$my_text  .= '<span style="color:' . $this->storeSuccessColor . '">';
				$my_text  .= Text::sprintf(
					'Created new matchplayer data. MatchID: %1$s - Player: %2$s,%3$s - Position: %4$s - oldTeamPlayerID : %5$s - newTeamPlayerID : %6$s',
	...
	private function _importMatchStaff()
	{
		...
		foreach ($this->_datas['matchstaff'] as $key => $matchstaff)
		{
			...
			)
			{
				$my_text .= '<span style="color:red">';
				$my_text .= Text::sprintf(
						'Skipping import of MatchStaff-ID [%1$s]. Old-MatchID: [%2$s] - Old-StaffID: [%3$s]',
						"</span><strong>$oldID</strong><span style='color:red'>",
						"</span><strong>$oldMatchID</strong><span style='color:red'>",
						"</span><strong>$oldTeamStaffID</strong>"
					) . '<br />';
				continue;
			}
			...
			try
			{
				...
				$my_text  .= '<span style="color:' . $this->storeSuccessColor . '">';
				$my_text  .= Text::sprintf(
					'Created new matchstaff data. MatchID: %1$s - Staff: %2$s,%3$s - Position: %4$s',
	...
	private function _importMatchReferee()
	{
		...
		foreach ($this->_datas['matchreferee'] as $key => $matchreferee)
		{
			...
			)
			{
				$my_text .= '<span style="color:red">';
				$my_text .= Text::sprintf(
						'Skipping import of MatchReferee-ID [%1$s]. Old-MatchID: [%2$s] - Old-RefereeID: [%3$s]',
						"</span><strong>$oldID</strong><span style='color:red'>",
						"</span><strong>$oldMatchID</strong><span style='color:red'>",
						"</span><strong>$oldProjectRefereeID</strong>"
					) . '<br />';
				continue;
			}
			...
			try
			{
				...
				$my_text  .= '<span style="color:' . $this->storeSuccessColor . '">';
				$my_text  .= Text::sprintf(
					'Created new matchreferee data. MatchID: %1$s - Referee: %2$s,%3$s - Position: %4$s',
	...
	private function _importMatchEvent()
	{
		...
		foreach ($this->_datas['matchevent'] as $key => $matchevent)
		{
			...
			try
			{
				...
				$my_text    .= '<span style="color:' . $this->storeSuccessColor . '">';
				$my_text    .= Text::sprintf(
					'Created new matchevent data. MatchID: %1$s - Player: %2$s,%3$s - Eventtime: %4$s - Event: %5$s',
	...
	private function _importPositionStatistic()
	{
		...
		foreach ($this->_datas['positionstatistic'] as $key => $positionstatistic)
		{
			...
			if ($object = Factory::getDbo()->loadObject())
			{
				$my_text .= '<span style="color:' . $this->existingInDbColor . '">';
				$my_text .= Text::sprintf(
					'Using existing positionstatistic data. Position: %1$s - Statistic: %2$s',
			...
			else
			{
				...
				else
				{
					$my_text .= '<span style="color:' . $this->storeSuccessColor . '">';
					$my_text .= Text::sprintf(
						'Created new position statistic data. Position: %1$s - Statistic: %2$s',
	...
	private function _importMatchStaffStatistic()
	{
		...
		$my_text = '';
		if (!isset($this->_datas['matchstaffstatistic']) || count($this->_datas['matchstaffstatistic']) == 0)
		{
			return true;
		}
		...
		foreach ($this->_datas['matchstaffstatistic'] as $key => $matchstaffstatistic)
		{
			...
			else
			{
				...
				$my_text .= '<span style="color:' . $this->storeSuccessColor . '">';
				$my_text .= Text::sprintf(
					'Created new match staff statistic data. StatisticID: %1$s - MatchID: %2$s - Player: %3$s,%4$s - Team: %5$s - Value: %6$s',
	...
	{
		$my_text = '';
		if (!isset($this->_datas['matchstatistic']) || count($this->_datas['matchstatistic']) == 0)
		{
			return true;
		}
		...
		foreach ($this->_datas['matchstatistic'] as $key => $matchstatistic)
		{
			...
			else
			{
				...
				$my_text .= '<span style="color:' . $this->storeSuccessColor . '">';
				$my_text .= Text::sprintf(
					'Created new match statistic data. StatisticID: %1$s - MatchID: %2$s - Player: %3$s,%4$s - Team: %5$s - Value: %6$s',
	...
	{
		$my_text = '';
		if (!isset($this->_datas['treeto']) || count($this->_datas['treeto']) == 0)
		{
			return true;
		}
	...
	{
		$my_text = '';
		if (!isset($this->_datas['treetonode']) || count($this->_datas['treetonode']) == 0)
		{
			return true;
		}
	...
	{
		$my_text = '';
		if (!isset($this->_datas['treetomatch']) || count($this->_datas['treetomatch']) == 0)
		{
			return true;
		}
	...
	private function _beforeFinish()
	{
		...
		if ($checked_fav_teams != '')
		{
			...
			{
				if (isset($this->_convertTeamID[$value]))
				{
					$t_fav_team .= $this->_convertTeamID[$value] . ',';
				}
			}
		...
		foreach ($result_pt as $proteam)
		{
			...
			foreach ($result_tp as $team_member)
			{
				...
				if ($new_player_id)
				{
					...
					$my_text .= '<span style="color:' . $this->existingInDbColor . '">';
					$my_text .= Text::sprintf(
						'spieler vorhanden season_person_id: alte id: %1$s - saison: %2$s - neue id: %3$s',
				...
				if ($new_match_player_id)
				{
					...
					$my_text .= '<span style="color:' . $this->existingInDbColor . '">';
					$my_text .= Text::sprintf(
						'spieler vorhanden season_team_person_id: alte id: %1$s - saison: %2$s - neue id: %3$s - team id: %4$s',
						"</span><strong>$team_member->person_id</strong>",
						"<strong>$this->_season_id</strong>",
				...
				else
				{
					...
					else
					{
						...
						$my_text             .= '<span style="color:' . $this->storeSuccessColor . '">';
						$my_text             .= Text::sprintf(
							'spieler nicht vorhanden und angelegt season_team_person_id: alte id: %1$s - saison: %2$s - neue id: %3$s - team id: %4$s',
							"</span><strong>$team_member->person_id</strong>",
							"<strong>$this->_season_id</strong>",
							"<strong>$new_match_player_id</strong>",
							"<strong>$new_team_id</strong>"
				...
				else
				{
					...
					if (!sportsmanagementModeldatabasetool::runJoomlaQuery())
					{
						$my_text .= '<span style="color:' . $this->existingInDbColor . '">';
						$my_text .= Text::sprintf(
							'spieler vorhanden _person_project_position: person id: %1$s - projekt: %2$s - project_position_id: %3$s',
							"</span><strong>$team_member->person_id</strong>",
					...
					else
					{
						$my_text .= '<span style="color:' . $this->storeSuccessColor . '">';
						$my_text .= Text::sprintf(
							'spieler nicht vorhanden und angelegt _person_project_position: person id: %1$s - projekt: %2$s - project_position_id: %3$s',
							"</span><strong>$team_member->person_id</strong>",
							"<strong>$this->_project_id</strong>",
							"<strong>$team_member->project_position_id</strong>"
						);
			...
			foreach ($result_tp as $team_member)
			{
				...
				if ($new_staff_id)
				{
					...
					$my_text .= '<span style="color:' . $this->existingInDbColor . '">';
					$my_text .= Text::sprintf(
						'trainer vorhanden season_person_id: alte id: %1$s - saison: %2$s - neue id: %3$s',
						"</span><strong>$team_member->person_id</strong>",
						"<strong>$this->_season_id</strong>",
				...
				if ($new_match_staff_id)
				{
					...
					$my_text .= '<span style="color:' . $this->existingInDbColor . '">';
					$my_text .= Text::sprintf(
						'trainer vorhanden season_team_person_id: alte id: %1$s - saison: %2$s - neue id: %3$s - team id: %4$s',
						"</span><strong>$team_member->person_id</strong>",
						"<strong>$this->_season_id</strong>",
				...
				else
				{
					...
					else
					{
						...
						$my_text            .= '<span style="color:' . $this->storeSuccessColor . '">';
						$my_text            .= Text::sprintf(
							'trainer nicht vorhanden und angelegt season_team_person_id: alte id: %1$s - saison: %2$s - neue id: %3$s - team id: %4$s',
							"</span><strong>$team_member->person_id</strong>",
							"<strong>$this->_season_id</strong>",
							"<strong>$new_match_staff_id</strong>",
							"<strong>$new_team_id</strong>"
				...
				else
				{
					...
					if (!sportsmanagementModeldatabasetool::runJoomlaQuery())
					{
						...
						$my_text .= Text::sprintf(
							'trainer vorhanden _person_project_position: person id: %1$s - projekt: %2$s - project_position_id: %3$s',
							"</span><strong>$team_member->person_id</strong>",
							"<strong>$this->_project_id</strong>",
							"<strong>$team_member->project_position_id</strong>"
						);
					...
					else
					{
						$my_text .= '<span style="color:' . $this->existingStaff . '">';
						$my_text .= Text::sprintf(
							'trainer nicht vorhanden und angelegt _person_project_position: person id: %1$s - projekt: %2$s - project_position_id: %3$s',
							"</span><strong>$team_member->person_id</strong>",
							"<strong>$this->_project_id</strong>",
							"<strong>$team_member->project_position_id</strong>"
						);
