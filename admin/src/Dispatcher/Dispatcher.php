...
final class Dispatcher extends ComponentDispatcher
{
    ...
    private const NATIVE_EDIT_VIEWS = ['eventtype','extrafield','season','sportstype'];
    private const SAFE_STANDARD_CRUD_CONTROLLERS = ['season','seasons','sportstype','sportstypes'];
    ...
    public function dispatch()
    {
        ...
        if (!$identity->authorise('core.manage', 'com_sportsmanagement')) { throw new \RuntimeException('Not authorised to manage SportsManagement.', 403); }
    ...
    private function isModernCrudTask(string $task,string $format): bool
    {
        ...
        if (!in_array($controller,self::SAFE_STANDARD_CRUD_CONTROLLERS,true)) return false;
    ...
    {
        if ($task !== 'display' || $layout !== 'default' || $format !== 'html') return false;
        if ($controller !== '' && $controller !== 'display') return false;
