...
final class Dispatcher extends ComponentDispatcher
{
    ...
    private const SAFE_STANDARD_CRUD_ACTIONS = [
        'add',
        'edit',
        'apply',
        'save',
    ...
    private function isModernCrudTask(string $task, string $format): bool
    {
        ...
        if (in_array($task, self::NATIVE_SPECIAL_TASKS, true)) {
            return true;
        }

        $controller = strtolower((string) strtok($task, '.'));

        if (in_array($controller, self::NATIVE_CRUD_CONTROLLERS, true)) {
            return true;
        }

        if (!in_array($controller, self::SAFE_STANDARD_CRUD_CONTROLLERS, true)) {
            return false;
        }

        $action = strtolower((string) substr($task, strlen($controller) + 1));

        return in_array($action, self::SAFE_STANDARD_CRUD_ACTIONS, true);
    }

    private function isModernEditDisplay(
        string $task,
        string $view,
        string $controller,
        string $layout,
    ...
    private function isModernDisplayRequest(
        string $task,
        string $view,
        string $controller,
        string $layout,
        ...
        $nativeView = 'Diddipoeler\\Component\\SportsManagement\\Administrator\\View\\'
            . ucfirst($view) . '\\HtmlView';
    ...
    private function dispatchLegacy(): void
    {
        ...
        if (!is_file($legacyEntryPoint)) {
            throw new \RuntimeException(
                'SportsManagement legacy administrator entry point not found.',
                500
            );
        }
