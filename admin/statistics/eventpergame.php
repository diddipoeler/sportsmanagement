...
//also the name of the associated xml file	
	...
	/**
	 * SMStatisticEventPergame::getSids()
	 * 
	 * @return
	 */
	function getSids()
	...
	/**
	 * SMStatisticEventPergame::getQuotedSids()
	 * 
	 * @return
	 */
	...
	$app = JFactory::getApplication();
        ...
        $query_num->join('INNER','#__'.COM_SPORTSMANAGEMENT_TABLE.'_match_event AS me ON me.teamplayer_id = tp.id AND me.event_type_id IN ('. implode(',', $sids) .')');
        $query_num->join('INNER','#__'.COM_SPORTSMANAGEMENT_TABLE.'_match AS m ON m.id = me.match_id AND m.published = 1 ');
...
//					. '   AND p.published = 1 ';
	...
	function getTeamsRanking($project_id, $limit = 20, $limitstart = 0, $order = NULL)
	{
        ...
        $query_num->from('#__'.COM_SPORTSMANAGEMENT_TABLE.'_team_player AS tp ');
        $query_num->join('INNER','#__'.COM_SPORTSMANAGEMENT_TABLE.'_project_team AS pt ON pt.id = tp.projectteam_id ');
