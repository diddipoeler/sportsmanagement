<?php
/**
 * SportsManagement administrator Teams template for Joomla 5/6.
 *
 * @version    5.6.0
 * @author     diddipoeler
...
$assets->registerAndUseScript(
    'com_sportsmanagement.admin.teams',
    'administrator/components/com_sportsmanagement/assets/js/teams.js',
    ['version' => 'auto'],
    ['defer' => true],
    ['core', 'multiselect']
);
...
<form action="<?php echo Route::_('index.php?option=com_sportsmanagement&view=teams' . ($this->clubId > 0 ? '&club_id=' . $this->clubId : '')); ?>" method="post" name="adminForm" id="adminForm">
            ...
            <tbody>
                <?php foreach ($this->items as $i => $item) : ?>
                    ...
                    <tr>
                        ...
                        <td>
                            <?php if ($canEdit) : ?>
                                <a href="<?php echo Route::_('index.php?option=com_sportsmanagement&task=team.edit&id=' . (int) $item->id . ($this->clubId > 0 ? '&club_id=' . $this->clubId : '')); ?>"><?php echo $this->escape($item->name); ?></a>
                            <?php else : ?>
                                <?php echo $this->escape($item->name); ?>
                            <?php endif; ?>
                            <div class="small text-muted">
                                <?php echo $this->escape($item->short_name); ?>
                                <?php if ($item->info !== '') : ?> · <?php echo $this->escape($item->info); ?><?php endif; ?>
                            </div>
                        </td>
                        ...
                        <td>
                            <select class="form-select form-select-sm" name="agegroup<?php echo (int) $item->id; ?>" data-jsm-mark-row="<?php echo (int) $i; ?>">
                                <option value="0"></option>
                                <?php echo $renderOptions($this->inlineOptions['agegroups'], $item->agegroup_id); ?>
                            </select>
                        </td>
                        <td>
                            <select class="form-select form-select-sm" name="sportstype<?php echo (int) $item->id; ?>" data-jsm-mark-row="<?php echo (int) $i; ?>">
                                <option value="0"></option>
                                <?php echo $renderOptions($this->inlineOptions['sportstypes'], $item->sports_type_id); ?>
                            </select>
                        </td>
                        <td class="text-center"><?php if ($item->website) : ?><a href="<?php echo htmlspecialchars((string) $item->website, ENT_QUOTES, 'UTF-8'); ?>" target="_blank" rel="noopener noreferrer"><?php echo Text::_('JYES'); ?></a><?php else : echo Text::_('JNO'); endif; ?></td>
    ...
    <input type="hidden" name="club_id" value="<?php echo (int) $this->clubId; ?>">
