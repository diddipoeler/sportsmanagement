<?php
/**
 * SportsManagement calendar rendering helpers.
 *
...
/**
 * Generate an event calendar in HTML format.
 *
 * This file is included by the module helper and therefore must not process
 * arbitrary POST requests at include time. AJAX requests belong to Joomla's
 * module/com_ajax dispatch layer instead of a stand-alone functions.php file.
 */
function getCalender($year = '', $month = ''): void
{
    $year = filter_var($year, FILTER_VALIDATE_INT, ['options' => ['min_range' => 1970, 'max_range' => 9999]]) ?: (int) date('Y');
    $month = filter_var($month, FILTER_VALIDATE_INT, ['options' => ['min_range' => 1, 'max_range' => 12]]) ?: (int) date('m');
        ...
        <section class="title-bar">
            ...
            <div class="title-bar__month">
                <select class="month-dropdown">
                    <?php echo getMonthList($month); ?>
                </select>
            </div>
            <div class="title-bar__year">
                <select class="year-dropdown">
                    <?php echo getYearList($year); ?>
                </select>
            </div>
            <a href="javascript:void(0);"
        ...
        <section class="calendar__days">
            ...
            for ($box = 1; $box <= $boxDisplay; $box++) {
                if (($box >= $currentMonthFirstDay || $currentMonthFirstDay === 1) && $box <= $totalDaysOfMonthDisplay) {
                    ...
                    echo '<div class="' . $classes . '" data-calendar-date="' . htmlspecialchars($currentDate, ENT_QUOTES, 'UTF-8') . '">';
...
/**
 * Generate month options for the calendar selector.
 */
...
/**
 * Generate year options for the calendar selector.
 */
...
/**
 * Generate the event-list heading for a date.
 *
 * Actual match data is populated by the SportsManagement calendar connector;
 * this helper intentionally performs no direct request dispatch or database IO.
 */
