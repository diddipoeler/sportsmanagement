...
class JSMCalendar extends PHPCalendar
{
    ...
    public static function getMatches($month, $year): array
    {
        ...
        if ((int) self::$params->get('jevents', 0) === 1) {
            require_once __DIR__ . '/connectors/jevents_j5.php';
        ...
        if ((string) self::$params->get('livescore', '') !== '') {
            require_once __DIR__ . '/connectors/livescore_j5.php';
    ...
    public function matches_output($month, $year): array
    {
        ...
        foreach ($matches as $index => $row) {
            ...
            $date = modJSMCalendarHelperDate::fromValue($row['date'], $offset);
            if (!$date) {
                continue;
            }
            ...
            if (is_array($next) && isset($next['date'])) {
                $nextDate = modJSMCalendarHelperDate::fromValue($next['date'], $offset);
...
/** Small shared date factory used by the renderer. */
final class modJSMCalendarHelperDate
{
