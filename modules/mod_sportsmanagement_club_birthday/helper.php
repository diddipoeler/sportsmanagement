<?php
/** Joomla 5/6 runtime helper for club anniversaries. */
...
class modSportsmanagementClubBirthdayHelper
{
    ...
    {
        $clubs = self::getClubs((int) $params->get('limit', 0), $params->get('s', []));
        if (count($clubs) > 1) {
            $clubs = self::jsm_birthday_sort($clubs, (int) $params->get('sort_order', 1));
        }
        foreach ($clubs as $club) {
            $club->birthday_text = self::birthdayText($club, $params);
        }
        return ['clubs' => $clubs, 'mode' => strtoupper((string) $params->get('mode', 'L'))];
    }

    public static function jsm_birthday_sort($array, $sort)
    {
        $direction = (int) $sort < 0 ? -1 : 1;
        usort($array, static fn(object $a, object $b): int => (((int) $a->age <=> (int) $b->age) * $direction));
        return $array;
    }

    public static function getClubs($limit, $seasonIds)
    {
        $db = self::database();
        $seasonIds = self::normaliseIds($seasonIds);
        $query = $db->getQuery(true)
            ->select([
                'c.id', 'c.country', 'c.founded', 'c.name', 'c.alias', 'c.founded_year',
                'c.logo_big AS picture', 'c.founded_timestamp', 'pt.project_id',
        ...
        usort($clubs, static fn(object $a, object $b): int => ((int) $a->days_to_birthday <=> (int) $b->days_to_birthday));
        $limit = max(0, (int) $limit);
        return $limit > 0 ? array_slice($clubs, 0, $limit) : $clubs;
    }

    private static function birthdayText(object $club, Registry $params): string
    {
        $when = match ((int) $club->days_to_birthday) {
            0 => (string) $params->get('todaymessage', ''),
            1 => (string) $params->get('tomorrowmessage', ''),
            default => str_replace('%DAYS_TO%', (string) $club->days_to_birthday, trim((string) $params->get('futuremessage', ''))),
        };
        $text = htmlspecialchars(trim(Text::_((string) $params->get('birthdaytext', ''))), ENT_QUOTES, 'UTF-8');
        $birth = new DateTimeImmutable((string) $club->date_of_birth, self::timezone());
        $next = new DateTimeImmutable((string) $club->year . '-' . (string) $club->daymonth, self::timezone());
        return strtr($text, [
    ...
    private static function clubLink(int $projectId, int $clubId): string
    {
        ...
        return Route::_('index.php?' . http_build_query(['option' => 'com_sportsmanagement', 'view' => 'clubinfo', 'p' => $projectId, 'cid' => $clubId]), false);
    }
    ...
    private static function pictureUrl(string $picture, Registry $params): string
    {
        ...
        return $placeholder !== '' && is_file(JPATH_ROOT . '/' . $placeholder) ? Uri::root() . $placeholder : '';
    }
    ...
    {
        try {
            return new DateTimeZone((string) Factory::getApplication()->get('offset', 'UTC'));
        } catch (Throwable) {
