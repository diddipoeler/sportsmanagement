...
require_once __DIR__ . '/native/QueryTrait.php';
require_once __DIR__ . '/native/FormatTrait.php';
require_once __DIR__ . '/native/LinkTrait.php';
        ...
        try {
            ...
            foreach ($this->loadMatches($db, $params, $projects) as $match) {
                ...
                $row = [
                    ...
                    'referees' => (int) $params->get('show_referee', 1) === 1 ? $this->referees($db, (int) $match->match_id, (int) $params->get('referee_name_format', 0)) : [],
                    'spectators' => (int) $params->get('show_spectators', 0) === 1 ? (int) ($match->crowd ?? 0) : 0,
                    'links' => $this->matchLinks($match, $params),
                    'navigation' => (int) $params->get('next_last', 0) > 0 ? $this->neighborLinks($db, $match, $params, $projects) : [],
                ];
