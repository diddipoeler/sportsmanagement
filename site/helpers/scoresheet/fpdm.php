...
//					$this->Error("set_field_value failed as invalid valuetype $type for object $object_id");
		...
		function fix_xref_start()
		{
			...
			if ($this->check_mode)
			{ //Compared calculated value with position value read direct from file
		...
		function _extract_pdf_definition_value($name, $line, &$match)
		{
			...
			if (!$value)
			{ //value is concatained with name: /name/value
...
//									if(!array_key_exists("default",$values)) $this->Error("Cannot find default value (/DV) for field $name");
							...
							else
							{
								...
								if (array_key_exists("length", $stream))
								{ //length is mandatory

									if (preg_match("/\b(stream|endstream)\b/", $CurLine, $match))
									{

										if (!array_key_exists("filters", $stream))
										{//filter type is optional, if none is given, its standard
					...
					switch ($xref_table)
					{
						...
						default:
							...
							if ($refs_count > 0)
							{
								...
								if ($refs_count == 1)
								{//Last one , due to the shift, is the trailer
							...
							{ //We are inside the trailer

								if ($trailer_table == 1)
								{ //should be <<

									if (trim($CurLine) != '')
									{ //HACK: PDFCreator Version 1.0.0 has an extra CR after trailer
								...
								else if (($trailer_table > 0) && ((!is_null($id_def)) || preg_match("/^\/(Size|Root|Info|ID|DocChecksum)/", $CurLine, $match)))
								{
									...
									if ($id_def)
									{//we are inside the ID definition
