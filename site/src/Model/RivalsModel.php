    ...
    public function getOpponents(): array
    {
        ...
        $query = $db->createQuery()
            ->select([
                ...
                "CONCAT_WS(':', " . $db->quoteName('pt1.id') . ', ' . $db->quoteName('t1.alias') . ') AS ' . $db->quoteName('projectteam1_slug'),
                "CONCAT_WS(':', " . $db->quoteName('pt2.id') . ', ' . $db->quoteName('t2.alias') . ') AS ' . $db->quoteName('projectteam2_slug'),
