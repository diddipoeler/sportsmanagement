<?php
/**
 * Joomla 5/6 site list view for persons.
 *
 * @version    5.6.0
 * @author     diddipoeler
 ...
 */
namespace Diddipoeler\Component\SportsManagement\Site\View\Allpersons;
...
final class HtmlView extends SportsManagementHtmlView
{
    ...
    public function display($tpl = null)
    {
        ...
        if (!$model instanceof AllpersonsModel) {
            throw new \RuntimeException('Allpersons view requires AllpersonsModel.', 500);
        }
        ...
        $this->sortColumn = (string) $this->state->get('filter_order', 'v.lastname');
        $this->lists = $this->buildFilterLists($model);
        $this->form = (object) ['limitField' => $this->pagination->getLimitBox()];

        $this->getDocument()->setTitle(Text::_('COM_SPORTSMANAGEMENT_ALLPERSONS_PAGE_TITLE'));

        parent::display($tpl);
    }

    private function buildFilterLists(AllpersonsModel $model): array
    {
        $options = [
