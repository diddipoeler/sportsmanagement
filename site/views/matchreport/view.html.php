...
class sportsmanagementViewMatchReport extends sportsmanagementView
{
    ...
    public function init()
    {
        ...
        if (!$this->match) {
            $this->document->setTitle(Text::_('COM_SPORTSMANAGEMENT_MATCHREPORT_PAGE_TITLE'));
            return;
        }
        ...
        $this->match_article = $this->model->getMatchArticle(
            (int) ($this->match->content_id ?? 0),
            $matchId,
            (int) ($project->category_id ?? 0)
        );
        $this->round = $this->model->getRound();
        $this->team1 = $this->model->getProjectTeamById((int) ($this->match->projectteam1_id ?? 0));
        $this->team2 = $this->model->getProjectTeamById((int) ($this->match->projectteam2_id ?? 0));
        $this->team1_club = $this->team1 ? $this->model->getClubinfo((int) ($this->team1->club_id ?? 0)) : null;
        $this->team2_club = $this->team2 ? $this->model->getClubinfo((int) ($this->team2->club_id ?? 0)) : null;
        ...
        $this->matchreferees = $this->model->getMatchReferees($matchId);
        $this->matchcommentary = $this->model->getMatchCommentary($matchId);
        $this->substitutes = $this->model->getMatchSubstitutions($matchId);
        $this->eventtypes = $this->model->getEventTypes();
        $sortEventsDesc = !empty($this->config['sort_events_desc']);
        $this->matchevents = $this->model->getMatchEvents($matchId, true, $sortEventsDesc);
        $this->playground = $this->model->getPlayground((int) ($this->match->playground_id ?? 0));
        $this->stats = $this->model->getProjectStats();
        ...
        $this->formation1 = $this->extended2 ? $this->extended2->getValue('formation1') : null;
        $this->formation2 = $this->extended2 ? $this->extended2->getValue('formation2') : null;
        $this->youtube = $this->extended2 ? $this->extended2->getValue('youtube') : null;
        ...
        $this->schemahome = $this->model->getPlaygroundSchema($this->formation1, 'heim');
        $this->schemaaway = $this->model->getPlaygroundSchema($this->formation2, 'gast');

        if (!empty($this->config['show_pictures'])) {
    ...
    function showSubstitution_Timelines($sub = 0,$projectteam_id = 'projectteam1_id')
    {
        ...
        foreach ($this->substitutes as $sub)
        {
            ...
            {
                if (!isset($substitutioncounter2[$sub->in_out_time]))
                {
                    $substitutioncounter2[$sub->in_out_time] = 0;
                }
