<?php
/**
 * SportsManagement results edit row.
 */
...
$editMatchRoute = static function (
    string $layout,
    int $teamId = 0,
    int $doubleEvents = 0
) use (
...
?>
<tr id="result-<?php echo (int) $match->id; ?>" class="row-result">
    <td valign="top">
        <?php
    ...
    <td valign="top">
        ...
        if (!$saveshortall) {
            echo HTMLHelper::_(
                ...
                '',
                ['title' => Text::_('JLIB_RULES_NOT_ALLOWED')]
            );
        } else {
    ...
    <?php if ($this->project->project_type == 'DIVISIONS_LEAGUE') { ?>
        <td style="text-align:center;">
            ...
            <input type="hidden" name="division_id<?php echo $thismatch->id; ?>"
                   value="<?php echo (int) ($match->divhomeid ?? 0); ?>"/>
        </td>
    <?php } ?>

    <?php if ($this->config['show_edit_match_number']) { ?>
    ...
    <?php if ($this->config['show_edit_match_date']) { ?>
    ...
    <?php if ($this->config['show_edit_match_time']) { ?>
    ...
    <?php if ($this->config['show_edit_match_time_present']) { ?>
        <td align="center" class="nowrap" valign="top">
            <input type="text" style="font-size:9px;" size="3" name="time_present<?php echo $thismatch->id; ?>"
                   value="<?php echo htmlspecialchars(substr((string) ($thismatch->time_present ?? ''), 0, 5), ENT_QUOTES, 'UTF-8'); ?>"
                   class="inputbox" onchange="document.getElementById('cb<?php echo $i; ?>').checked=true;"/>
        </td>
    <?php } ?>
    ...
    <td>
        ...
        if ((!$userIsTeamAdmin && empty($match->allowed)) || !$saveshortall) {
    ...
    if ($this->config['results_below']) {
        ...
        if ($this->project->allow_add_time) {
            ...
            <td valign="top" align="center">
                <span id="ot<?php echo $thismatch->id; ?>"
                      style="visibility:<?php echo ((int) ($thismatch->match_result_type ?? 0) > 0) ? 'visible' : 'hidden'; ?>">
        ...
        if ($this->project->allow_add_time) {
            ...
            $onChange = "document.getElementById('cb{$i}').checked=true;"
                . "var overtime=document.getElementById('ot{$thismatch->id}');"
                . "if(overtime){overtime.style.visibility=this.selectedIndex===0?'hidden':'visible';}";
            ?>
            <td align="center" valign="top">
                <?php
                echo HTMLHelper::_(
        ...
        <td valign="top" style="text-align:center;">
            <input <?php echo $saveshortall ? '' : 'disabled="disabled"'; ?> type="checkbox"
                   name="published<?php echo $thismatch->id; ?>" id="cbp<?php echo $thismatch->id; ?>"
