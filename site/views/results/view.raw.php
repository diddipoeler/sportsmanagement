...
//		$document->addStyleSheet($css);
	...
	public function getTeamClubIcon($team,$type=1,$attribs=array())
	{
		...
		if ($type==1)
		{
			...
			{
				$image=HTMLHelper::image(Uri::root().sportsmanagementHelper::getDefaultPlaceholder("clublogosmall"),$title,$attribs);
			}
		}
	...
	*/
	...
	function prepareEventsOutput($match_id)
	{
		...
		$attribs=array(	"title" => $imgTitle,
		 		"id" => 'events-'. $match_id,
		 		"class" => "eventstoggle");
		$img=HTMLHelper::image(Uri::root().'media/com_sportsmanagement/jl_images/events.png',$imgTitle,$attribs);
		return $img;
	}
	...
	function showEventsContainerInResults($matchInfo,$projectevents,$matchevents,$substitutions=null,$config)
	{
		...
		if ($this->config['use_tabs_events'])
		{
			...
			if (!empty($substitutions))
			{
				...
				$pic_tab	= Uri::root().'images/com_sportsmanagement/database/events/'.$this->project->fs_sport_type_name.'/subst.png';
