 ...
 */
        ...
        <?php if (empty($match->match_number)) {$match->match_number='-';
    ...
    <?php
       ...
       <td width="10%" id="teamplan-spieluhrzeit" ><?php echo sportsmanagementHelperHtml::showMatchTime(
        ...
        if ($this->config['show_time_present']) {
        ?>
       <td width='10%'><?php if (empty($match->time_present)) {$match->time_present='-';
 ...
 */
    ...
    <?php
        ...
        if ($this->config['show_attendance_column']) {
        ?>
       <td class="center"><?php if ($match->crowd == 0) {$match->crowd='';
      } echo $match->crowd; ?></td>
        <?php
        }
        ?>
